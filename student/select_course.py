...
#     #返回该学生的选课记录
#     Eqs = E.objects.values(xh=curStudent.xh)
#     # 将 QuerySet 对象 转化为 list 类型
#     Elist = list(Eqs)
#     return JsonResponse({'ret': 0, 'retlist': Elist})
...
#    curStudent = S.objects.get(xh=request.session['member_id'])
    ...
    for i in selectedcourse:
        ...
        if i.kh == selectinfo['kh']:
            flag = 1
            return JsonResponse({'ret': 1, 'msg': '该课已选，选课失败'})
